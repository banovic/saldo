...
//   - if no exchange rate is set, amounts are identical (same amount and currency)
//   - if exchange rate is set, the amounts are in different currencies
//   - if the amounts have same currency, they are identical (same minor units)
func (p Posting) Validate(er ExchangeRate) error {
	...
	if er.IsZero() && p.FunctionalAmount != p.TransactionAmount {
	...
	if !er.IsZero() && p.FunctionalAmount.Currency == p.TransactionAmount.Currency {
	...
	// TODO!!! - when ExchangeRate is implemented: check if functional amount is correctly calculated from transaction amount and exchange rate
	return nil
}
